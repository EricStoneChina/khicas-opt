...
signatures=('  static bool integrate_log_trig_quarters(', '  static bool integrate_log_product_zeta(', '  static int integration_quarter_sigma(', '  static bool integrate_log_trig_sum(', '  static bool integrate_atan_log_measure(')
helpers=args.helpers.read_text() if args.helpers else ''.join(ib.function(s,sig) for sig in signatures)
text='#include "giacPCH.h"\nnamespace giac {\n'
for sig in ('  void decompose_prod(', '  gen extract_cst(', '  static bool integration_rational(', '  static gen integration_syntax(', '  static bool integration_power(', '  static bool integration_one_plus(', '  static gen integration_coefficient(', '  static bool integration_monomial('):text+=ib.function(s,sig)
text+=helpers
calls=' || '.join(name+'(e,x,lo,hi,r,contextptr)' for name in ('integrate_log_trig_quarters','integrate_log_product_zeta','integrate_log_trig_sum','integrate_atan_log_measure'))
text+='bool cycle6_log_rule(const gen &f,const gen &x,const gen &lo,const gen &hi,gen &r,GIAC_CONTEXT){gen e=integration_syntax(f,contextptr);return '+calls+';}\n}\n'
report=[]
with tempfile.TemporaryDirectory(prefix='khicas-cycle6-log-') as tmp:
 ...
 unit=d/'unit';subprocess.run(flags+[str(d/'rules.cc'),str(ib.ROOT/'tests/cycle6_log_integrals.cc')]+libs+['-pthread','-o',str(unit)],check=True)
 ...
 if args.full:
  ...
  (Path('/tmp')/'khicas-cycle6-log-yintg.cc').write_text(candidate)
