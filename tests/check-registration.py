...
current = [(ROOT / f).read_text() for f in ('static_lexer.h', 'static_lexer_.h')]
original = [subprocess.check_output(['git', 'show', f'{UPSTREAM}:{f}'],
            cwd=ROOT, text=True) for f in ('static_lexer.h', 'static_lexer_.h')]
for release in (False, True):
    ...
    assert sorted(row for row in rows if row[0] not in COMMANDS) == sorted(
        table(*original, release)), 'Existing command metadata/aliases changed'
