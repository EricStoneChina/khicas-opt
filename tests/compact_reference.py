...
def verify_binomial_reference(case, printed):
    ...
    import sympy as sp
    spec=importlib.util.spec_from_file_location('frozen_cycle8',Path(__file__).with_name('generate-coverage-cycle8.py'))
    module=importlib.util.module_from_spec(spec);spec.loader.exec_module(module)
    original=module.build_corpus()['cases'][1]
    assert all(case[k]==original[k] for k in ('id','f','expected'))
    x=sp.Symbol('x',real=True);t=sp.Symbol('t',positive=True);n=sp.Symbol('n',integer=True,positive=True)
    ...
    expected=parse(case['expected']);actual=parse(printed)
    ...
    mapping={power:sp.Dummy() for power in powers if power.exp.is_Integer and abs(power.exp)>=32}
    assert sp.cancel(actual.xreplace(mapping)-expected.xreplace(mapping))==0
    return {'exit':0,'stderr':'CHECK_METHOD exact whole-power identity against independently proved generic binomial primitive\nCHECK exact\n'}
