...
for sig in ('  void decompose_prod(', '  gen extract_cst(', '  static bool integration_rational(', '  static gen integration_syntax(', '  static bool integration_power(', '  static gen integration_coefficient('):
    text+=function(s,sig)
...
with tempfile.TemporaryDirectory(prefix='khicas-user-arc-') as tmp:
    ...
    subprocess.run(flags+[str(p/'rules.cc'),str(ROOT/'tests/user_arc_integrals.cc')]+libs+['-o',str(p/'test')],check=True)
