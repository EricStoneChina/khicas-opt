...
parser.add_argument('--helpers',type=Path,help='Read pending helper definitions before source integration')
...
for sig in ('  void decompose_prod(', '  gen extract_cst(',
    ...
    text+=function(s,sig)
if args.helpers:
    text+=args.helpers.read_text()
else:
    for sig in ('  static bool integrate_log_zeta(', '  static bool integrate_hyperbolic_log(',
...
with tempfile.TemporaryDirectory(prefix='khicas-cycle3-log-') as tmp:
    ...
    subprocess.run(flags+[str(p/'rules.cc'),str(ROOT/'tests/cycle3_log_integrals.cc')]+libs+['-o',str(p/'test')],check=True)
