...
CASES=[
 ...
 'integrate(x*sin(x),x)', 'integrate(x*exp(x^2),x)',
 'integrate(ln(x)^3,x)', 'integrate(1/(1+x^4),x)',
 'integrate(exp(-x^2),x)', 'integrate(1/sqrt(1-x^2),x)',
 'integrate(x^9-2*x+3,x)', 'integrate(x^2,x,0,1)',
 'integrate(exp(x),x,0.0,1.0)',
 'integrate(sin(t),t)', 'integrate(sin(x),x=0..pi)',
]
with tempfile.TemporaryDirectory(prefix='khicas-integrate-') as tmp:
    ...
    for ref in ('baseline','current'):
        ...
        results=[]
        for expr in CASES:
            r=subprocess.run([str(exe),expr],capture_output=True,text=True,timeout=30)
            assert r.returncode==0,(ref,expr,r.stderr)
            results.append(r.stdout.strip())
        if ref=='baseline': reference=results
        else: assert results==reference,[(e,a,b) for e,a,b in zip(CASES,reference,results) if a!=b]
        timings=[];calls=[]
        for _ in range(7):
        ...
        if ref=='current':
            shortcuts=[
              ...
              ('integrate(x^2024*(1-x^2025)^2025,x=0..1)','1/(2025*2026)','definite'),
              ...
              ('integrate(abs(x-1)/(abs(x-2)+abs(x-3)),x,0,4)','2+9*ln(3)/4-3*ln(5)/4','definite'),
              ('integrate(abs(x-1)/(abs(x-2)+abs(x-3)),x,4,0)','-2-9*ln(3)/4+3*ln(5)/4','definite'),
            ...
            for expr,expected,mode in shortcuts:
                ...
                p=subprocess.run([str(exe),expr,expected,mode,integrand],capture_output=True,text=True,timeout=10)
                assert p.returncode==0 and 'CHECK exact' in p.stderr,(expr,p.stdout,p.stderr)
            report[ref]['shortcut_cases']=len(shortcuts)
    assert max(report['current']['parser_calls'])<min(report['baseline']['parser_calls'])
    print(json.dumps(report,indent=2))
    print('PASS: actual integral modules agree; bounded-stack regression and parser count')
