...
for sig in ('  void decompose_prod(', '  gen extract_cst(',
 ...
 '  static bool integration_mellin_monomial(', '  static bool integrate_loglog_mellin('):text+=function(s,sig)
if args.helpers:text+=args.helpers.read_text()
else:
 for sig in ('  static bool integrate_atan_frullani(', '  static bool integration_log_slope(', '  static bool integrate_loglog_frullani('):text+=function(s,sig)
