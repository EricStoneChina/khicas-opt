...
with tempfile.TemporaryDirectory(prefix='khicas-equation-extended-') as tmp:
 ...
 for name in ('equation_conversions_extended.cc','equation_polar_compact.cc','equation_conversions.cc'):
  subprocess.run(flags+[str(source),str(ROOT/'tests'/name)]+libs+['-o',str(d/'test')],check=True)
