...
if args.helpers:text+=args.helpers.read_text()
 ...
 for sig in ('  static bool integrate_atan_frullani(', '  static bool integrate_loglog_mellin('):text+=function(s,sig)
...
with tempfile.TemporaryDirectory(prefix='khicas-user-log-') as tmp:
 ...
 subprocess.run(flags+[str(p/'rules.cc'),str(ROOT/'tests/user_log_integrals.cc')]+libs+['-o',str(p/'test')],check=True)
