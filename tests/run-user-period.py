...
for sig in ('  void decompose_prod(', '  gen extract_cst(', '  static bool integration_rational(', '  static gen integration_syntax(', '  static bool integration_power(', '  static gen integration_coefficient('):text+=function(s,sig)
if args.helpers:text+=args.helpers.read_text()
else:
    for sig in ('  static bool integration_period_real_bound(', '  static bool integrate_reciprocal_trig_period('):text+=function(s,sig)
...
with tempfile.TemporaryDirectory(prefix='khicas-user-period-') as tmp:
    ...
    subprocess.run(flags+[str(p/'rules.cc'),str(ROOT/'tests/user_period_integrals.cc')]+libs+['-o',str(p/'test')],check=True)
