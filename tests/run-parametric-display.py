...
with tempfile.TemporaryDirectory(prefix='khicas-parametric-display-') as tmp:
 ...
 flags=[os.environ.get('CXX','c++'),'-std=c++11','-O2','-DHAVE_CONFIG_H','-DGIAC_GENERIC_CONSTANTS','-Wno-deprecated-declarations','-I',os.environ.get('GIAC_INCLUDE','/usr/include/giac'),'-I',str(d)]+shlex.split(os.environ.get('CXXFLAGS',''))
 libs=shlex.split(os.environ.get('LDFLAGS',''))+['-lgiac']
 libs += shlex.split(os.environ.get('GIAC_NUMERIC_LIBS', '-lgmp -lmpfr'))
 subprocess.run(flags+[str(d/'kconvert.cc'),str(ROOT/'tests/parametric_display.cc')]+libs+['-o',str(d/'test')],check=True)
