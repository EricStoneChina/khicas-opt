...
old=subprocess.check_output(['git','show','checkpoint/integration-cycle2-2026a:input_lexer.ll'],cwd=ROOT,text=True)
new=(ROOT/'input_lexer.ll').read_text()
...
def get(s):
 ...
 return s[a:b]
assert get(new)==get((ROOT/'input_lexer.cc').read_text()), 'Regenerated lexer is out of sync'
source=r'''
#include "giacPCH.h"
...
'''
inputs=['','x','1+2','sin(x','[1,2','x)','x]','x:=1:;:;','"a..b"','x\\\ny','1e−3','a→b','x≤3','"escaped\\\"text"','/* comment */x','// comment\rx+1']
for corpus in ('calculus-corpus.json','generalization-corpus.json','generalization-cycle2.json','generalization-cycle3.json'):
 inputs += [c['f'] for c in json.loads((ROOT/'tests'/corpus).read_text())['cases']]
inputs += ['x'*n for n in (6143,6144,8191,8192,9000)]
with tempfile.TemporaryDirectory(prefix='khicas-lexer-') as tmp:
 p=Path(tmp);(p/'test.cc').write_text(source);(p/'inputs.txt').write_text('\n'.join(inputs)+'\n')
 flags=[os.environ.get('CXX','c++'),'-std=c++11','-O1','-g','-fsanitize=address,undefined','-fno-omit-frame-pointer','-DHAVE_CONFIG_H','-DGIAC_GENERIC_CONSTANTS','-Wno-deprecated-declarations','-I',os.environ.get('GIAC_INCLUDE','/usr/include/giac')]+shlex.split(os.environ.get('CXXFLAGS',''))
 ...
 subprocess.run([str(p/'test'),str(p/'inputs.txt')],env=env,check=True,timeout=60)
