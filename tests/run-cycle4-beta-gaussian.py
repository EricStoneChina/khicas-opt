...
parser.add_argument('--replacement',type=Path,help='Pending Beta joint/log function replacements')
args=parser.parse_args()
s=(ROOT/'yintg.cc').read_text()
if args.replacement:
 candidate=args.replacement.read_text()
 for sig in ('  static gen integration_beta_joint(', '  static bool integrate_beta_log('):s=s.replace(function(s,sig),function(candidate,sig))
...
with tempfile.TemporaryDirectory(prefix='khicas-cycle4-beta-gaussian-') as tmp:
 ...
 subprocess.run(flags+[str(p/'rules.cc'),str(ROOT/'tests/cycle4_beta_gaussian_integrals.cc')]+libs+['-pthread','-o',str(p/'test')],check=True)
