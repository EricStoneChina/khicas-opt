...
from integration_build import ROOT,build,compiler_options
...
with tempfile.TemporaryDirectory(prefix='khicas-simplify-resources-') as tmp:
 ...
 subprocess.run(flags+[str(d/'simplify.cc'),str(ROOT/'tests/simplify-resource-boundary.cc')]+libs+['-pthread','-o',str(isolated)],check=True)
 ...
 for case in cases:
  ...
  for stack in ('normal','64'):
   ...
   output=r.stdout
 print('PASS: 34 complete-pipeline nested/large-power calls',flush=True)
