...
parser.add_argument('--helpers',type=Path,help='Read pending helper definitions')
args=parser.parse_args()
s=(ROOT/'yintg.cc').read_text()
text='#include "giacPCH.h"\nnamespace giac {\n'
for sig in ('  void decompose_prod(', '  gen extract_cst(', '  static bool integration_rational(', '  static gen integration_syntax(', '  static bool integration_power(', '  static gen integration_coefficient(', '  static bool integration_monomial('):text+=function(s,sig)
text+=args.helpers.read_text() if args.helpers else function(s,'  static bool integrate_mellin_two_binomials(')
...
with tempfile.TemporaryDirectory(prefix='khicas-cycle4-mellin-') as tmp:
    ...
    subprocess.run(flags+[str(p/'rules.cc'),str(ROOT/'tests/cycle4_mellin_integrals.cc')]+libs+['-o',str(p/'test')],check=True)
    subprocess.run([str(p/'test')],check=True,timeout=60)
    subprocess.run([str(p/'test')],check=True,timeout=60,env=dict(os.environ,KHICAS_TEST_STACK_KIB='64'))
