...
with tempfile.TemporaryDirectory(prefix='khicas-cycle7-tail-mellin-') as tmp:
 ...
 subprocess.run(flags+[str(d/'rules.cc'),str(ROOT/'tests/cycle7_tail_mellin.cc')]+shlex.split(os.environ.get('LDFLAGS',''))+['-lgiac','-pthread','-o',str(exe)],check=True,timeout=120)
 subprocess.run([str(exe)],check=True,timeout=60)
if args.verify:
 import mpmath as m
 m.mp.dps=80;errors=[]
 for a,b,q,tail in [(1,1,1,False),(2,3,1,False),(-2,3,1,False),(1,1,1,True),(2,3,1,True),(2,3,2,False),(1,2,'0.5',False)]:
  a,b,q=map(m.mpf,(a,b,q));got=m.quad(lambda x:x**(q-1)*(m.erfc(a*x**q) if tail else m.erf(a*x**q))*m.erfc(b*x**q),[0,1,m.inf])
  expected=((a+b-m.sqrt(a*a+b*b)) if tail else (m.sqrt(a*a+b*b)-b))/(q*a*b*m.sqrt(m.pi));errors.append(abs(got-expected))
 for A,B,q,power,r,n in [(1,1,2,0,'1.5',1),(1,4,2,0,'1.5',1),(1,1,2,1,2,1),(1,1,2,1,2,2),(1,1,1,0,2,1),(2,2,1,0,2,1),(1,1,2,0,'1.5',2),(1,1,2,1,2,4)]:
  A,B,q,power,r=map(m.mpf,(A,B,q,power,r));got=m.quad(lambda x:x**power*m.log(A+B*x**q)**n/(A+B*x**q)**r,[0,1,m.inf]);a=(power+1)/q
  expected=(-1)**n*m.diff(lambda z:A**(-z)*(A/B)**a*m.beta(a,z-a)/q,r,n);errors.append(abs(got-expected))
