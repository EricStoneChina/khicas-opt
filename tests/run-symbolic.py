...
def function(s,name):
    start=s.index('  void '+name+'(');end=s.index('{',start)+1;depth=1
    while depth:depth+=(s[end]=='{')-(s[end]=='}');end+=1
    return s[start:end]+'\n'
old=subprocess.check_output(['git','show','checkpoint/equations:zlin.cc'],cwd=ROOT,text=True)
new=(ROOT/'zlin.cc').read_text()
...
integrator=(ROOT/'yintg.cc').read_text()
for signature in ('  void decompose_prod(', '  gen extract_cst(',
                  '  static bool small_polynomial(', '  static int continuous_parity(',
                  '  static bool small_sparse_polynomial(', '  static bool integrate_sparse_atan(',
...
with tempfile.TemporaryDirectory(prefix='khicas-symbolic-') as tmp:
    p=Path(tmp);(p/'buffers.cc').write_text(text)
    (p/'integration_guard.h').write_bytes((ROOT/'integration_guard.h').read_bytes())
    ...
    subprocess.run(flags+[str(p/'buffers.cc'),str(p/'test.cc')]+libs+['-o',str(p/'test')],check=True)
