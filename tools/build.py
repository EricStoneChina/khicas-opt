...
def portable_makefile(text):
    ...
    text = text.replace("khelpfr.o: khelpfr.cc static_help.h mkhelp\n\t./mkhelp",
                        "khelpfr.o: khelpfr.cc .help-generated")
    text = text.replace("khelpen.o: khelpen.cc static_help.h\n\t./mkhelp",
...
def prepare(profile, compiler):
    ...
    if profile == "official":
        ...
        for path in sorted(ROOT.iterdir()):
            if path.is_file() and path.name not in (".git",):
...
def main():
    ...
    parser.add_argument("profile", choices=("official", "optimized"), nargs="?", default="optimized")
    ...
    bindir = next((path for path in candidates if (path / "sh3eb-elf-g++").exists()), None)
    if bindir is None:
        parser.error("Toolchain missing; see README.md and --tools-dir / TOOLS_DIR")
    env = dict(os.environ)
    env["PATH"] = f"{bindir}:{tools / 'bin'}:" + env.get("PATH", "")
    env["LD_LIBRARY_PATH"] = str(tools / "runtime") + ":" + env.get("LD_LIBRARY_PATH", "")
    compiler = subprocess.check_output(["sh3eb-elf-g++", "--version"], env=env, text=True)
    ...
    subprocess.run(["make", f"CASIOLOCAL={libraries}", f"-j{args.jobs}"] + args.targets,
